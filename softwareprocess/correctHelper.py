...
def checkAssumedLongFormat(assumedLong):
    if(not(isinstance(assumedLong, basestring))):
        return False
    else:
        if(assumedLong.count('d') != 1):
            return False
        posOfd = assumedLong.find('d')
        degString = assumedLong[0: posOfd]
        minString = assumedLong[posOfd + 1: len(assumedLong)]
